...
revision_and_size_to_model_map = {
    (
        "A",
        SIZE_3_5_INCH,
    ): TURING_MODEL,  # Can also be UsbPCMonitor 3.5, does not matter since protocol is the same
...
hw_lib_map = {
    "AUTO": "Automatic",
    "LHM": "LibreHardwareMonitor (admin.)",
    "PYTHON": "Python libraries",
    "STUB": "Fake random data",
    "STATIC": "Fake static data",
}
...
class TuringConfigWindow:
    def __init__(self):
        ...
        self.window.iconphoto(
            True, PhotoImage(file="assets/icons/monitor-icon-17865/64.png")
        )
        ...
        self.model_cb = ttk.Combobox(
            self.window,
            values=list(dict.fromkeys((revision_and_size_to_model_map.values()))),
            state="readonly",
        )
        ...
        self.brightness_warning_label = ttk.Label(
            self.window,
        ...
        sysmon_label = ttk.Label(
            self.window, text="System Monitor Configuration", font="bold"
        )
        ...
        self.edit_theme_btn = ttk.Button(
            self.window, text="Edit theme", command=lambda: self.on_theme_editor_click()
        )
        self.edit_theme_btn.place(x=310, y=490, height=50, width=130)

        self.save_btn = ttk.Button(
            self.window, text="Save settings", command=lambda: self.on_save_click()
        )
        self.save_btn.place(x=450, y=490, height=50, width=130)

        self.save_run_btn = ttk.Button(
            self.window, text="Save and run", command=lambda: self.on_saverun_click()
        )
    ...
    def save_config_values(self):
        ...
        self.config["display"]["REVISION"] = model_and_size_to_revision_map[
            (self.model_cb.get(), self.size_cb.get())
        ]
    ...
    def show_hide_brightness_warning(self, e=None):
        if (
            int(self.brightness_slider.get()) > 50
            and self.model_cb.get() == TURING_MODEL
            and self.size_cb.get() == SIZE_3_5_INCH
        ):
