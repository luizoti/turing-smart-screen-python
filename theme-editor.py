...
src.log.logger.setLevel(logging.NOTSET)  # Disable system monitor logging for the editor
...
from src.display import display  # Only import display after hardcoded config is set
...
if __name__ == "__main__":
    ...
    def draw_zone(x0, y0, x1, y1):
        ...
        if width > 0 and height > 0:
            label_zone.place(
                x=x + RGB_LED_MARGIN, y=y + RGB_LED_MARGIN, width=width, height=height
            )
        else:
    ...
    def on_button1_press_and_drag(event):
        ...
        label_coord.config(
            text="Drawing zone from [{},{}] to [{},{}]".format(x0, y0, x1, y1)
        )
    ...
    def on_button1_release(event):
        ...
        if x1 != x0 or y1 != y0:
            ...
            height = max(y0, y1) - min(y0, y1)
            label_coord.config(
                text="Zone: X={}, Y={}, width={} height={}".format(x, y, width, height)
            )
        else:
            # Display click coordinates
            label_coord.config(
                text="X={}, Y={} (click and drag to draw a zone)".format(
                    x0, y0, abs(x1 - x0), abs(y1 - y0)
                )
            )
    ...
    viewer.iconphoto(
        True, tkinter.PhotoImage(file="assets/icons/monitor-icon-17865/64.png")
    )
    viewer.geometry(
        str(display.lcd.get_width() + 2 * RGB_LED_MARGIN)
        + "x"
        + str(display.lcd.get_height() + 2 * RGB_LED_MARGIN + 40)
    )
    ...
    label_coord.place(
        x=0,
        y=display.lcd.get_height() + 2 * RGB_LED_MARGIN,
        width=display.lcd.get_width() + 2 * RGB_LED_MARGIN,
    )

    label_info = tkinter.Label(
        viewer, text="This preview will reload when theme file is updated"
    )
    label_info.place(
        x=0,
        y=display.lcd.get_height() + 2 * RGB_LED_MARGIN + 20,
        width=display.lcd.get_width() + 2 * RGB_LED_MARGIN,
    )

    label_zone = tkinter.Label(
        viewer, bg="#%02x%02x%02x" % tuple(map(lambda x: 255 - x, led_color))
    )
    ...
    while True:
        if os.path.getmtime(theme_file) > last_edit_time:
            logger.debug(
                "The theme file has been updated, the preview window will refresh"
            )
            refresh_theme()
            last_edit_time = os.path.getmtime(theme_file)
            ...
            led_color = config.THEME_DATA["display"].get(
                "DISPLAY_RGB_LED", (255, 255, 255)
            )
            if isinstance(led_color, str):
                led_color = tuple(map(int, led_color.split(", ")))
            viewer.configure(bg="#%02x%02x%02x" % led_color)
            label_zone.configure(
                bg="#%02x%02x%02x" % tuple(map(lambda x: 255 - x, led_color))
            )
