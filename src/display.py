...
class Display:
    def __init__(self):
        ...
        if config.CONFIG_DATA["display"]["REVISION"] == "A":
            ...
            self.lcd = LcdCommRevD(
                com_port=config.CONFIG_DATA["config"]["COM_PORT"],
                update_queue=config.update_queue,
            )
        elif config.CONFIG_DATA["display"]["REVISION"] == "SIMU":
            self.lcd = LcdSimulated(display_width=320, display_height=480)
        elif config.CONFIG_DATA["display"]["REVISION"] == "SIMU5":
            self.lcd = LcdSimulated(display_width=480, display_height=800)
        else:
            logger.error(
                "Unknown display revision '",
    ...
    def initialize_display(self):
        ...
        # Set orientation
        self.lcd.set_orientation(_get_theme_orientation())
    ...
    def turn_off(self):
        ...
        self.lcd.set_backplate_led_color(led_color=(0, 0, 0))
        ...
        if config.THEME_DATA.get("static_text", False):
            for text in config.THEME_DATA["static_text"]:
                ...
                self.lcd.DisplayText(
                    ...
                    font=config.THEME_DATA["static_text"][text].get(
